...
class LogFormScreenView extends StatefulHookConsumerWidget {
  final String trip_id;
  final String? uid;

  const LogFormScreenView({super.key, required this.trip_id, this.uid});

  @override
  LogFormScreenState createState() => LogFormScreenState();
}

class LogFormScreenState extends ConsumerState<LogFormScreenView> {
  ...
  Future<void> _loadLog() async {
    ...
    initialValues = {
      ...
      'type': log?.type ?? false,
      'driving_type': vehicle ? 2 : 1,
      'vehicle_id': log?.vehicle_id ?? '',
      'ruc': false,
      'company_type': company ? 2 : 1,
      'company_id': log?.company_id ?? '',
    };
    _isWork.value = !type;
    _drivingEnabled.value = vehicle;
    _selfEmployed.value = company;
